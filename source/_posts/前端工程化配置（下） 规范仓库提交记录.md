...
执行 `git commmit -m 'xxx'` 时，用来检查 `'xxx'` 是否满足固定格式的工具。
...
放入暂存区（`git add .`）以后，执行 `git cz` 启动 `cz-git` 适配器，它会读取你的 `commitlint.config.js` 配置，确认提交后会先调用 `pre-commit` hook 执行 `lint-staged` 检查暂存区的代码格式是否规范并修改。
